...
//    pcl::copyPointCloud( *alignedCloud, *cloudSourceWithNormal );
    ...
    corrEstNormShoot->setKSearch( 15 );
    ...
    corrRejSurNorm->setThreshold( 0.1 );
    ...
    // Set the transformation epsilon (criterion 2)
    icpNorm.setTransformationEpsilon (1e-6);

    icpNorm.setEuclideanFitnessEpsilon(1e-6);
...
pcl::PointCloud<PointTReg>::Ptr RegMeshPcd::registerPointClouds ( std::vector<pcl::PointCloud<PointTReg>::Ptr> & cloudVector ){
    ...
    for (int i = 0; i < cloudVector.size()-1; i++){
        ...
        cloudAlignedIcp = getIcpNormal( cloudSource, cloudTarget, maxCorrDist2, ransacStatOutThresh2, maxIterations2 );
...
pcl::PolygonMesh RegMeshPcd::generateMesh( pcl::PointCloud<pcl::PointXYZ>::Ptr p_cloud )
{
    ...
    pcl::io::saveVTKFile ("../DataSets/body/PointClouds/outputMesh.vtk", output);
